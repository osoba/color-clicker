...
document.addEventListener('DOMContentLoaded', () => {
	let WIDTH = 10, HEIGHT = 12, BOX_SIDE = 48, NUM_OF_COLORS = 7;

	if (window.localStorage) {
		WIDTH = Number(localStorage.getItem('width')),
			HEIGHT = Number(localStorage.getItem('height')),
			NUM_OF_COLORS = Number(localStorage.getItem('colors_no'));
	...
	canvas.width = (WIDTH * BOX_SIDE);
	canvas.height = (HEIGHT * BOX_SIDE);
